...
### A 组补充 · 浏览器端实测（2026-09-22，已由 Aria 跑完）

之前这一块一直空着（只验到「构建通过」，没验「界面能不能用」）。现已用**无头 Chrome**实测：

| 场景 | 实测结果 |
|---|---|
| 未登录直接开 `/dashboard` | 重定向到 `/login?redirect=/dashboard` —— 守卫生效，且**带上了回跳参数** ✓ |
| 输入错误密码 | 提示「账号或密码错误」，**停在登录页**不跳走 ✓ |
| 输入正确密码 | 跳 `/dashboard`，顶栏显示「云泽」；看板显示 消息总数 5 / 累计关注 2 / 取关 2 / 净增 0 / 活跃用户 1 ✓ |
| 登录后刷新页面 | 仍在 `/dashboard`（会话保持，没掉线）✓ |
| `/messages` 页面 | 渲染出真实 5 行流水（id 6/5/3/2…）✓ |

- **页面 JS 错误：0 条**
- `console.error` 仅 3 条，全部无害：1 条 `favicon.ico` 404（项目没放图标）+ 2 条 401（未登录探测与错误密码，均为预期）

> 复现方式：`puppeteer-core` 驱动系统已装的 Google Chrome（**不下载 Chromium**）。
> 前置：后端 8080 + 前端 `npm run dev`（5173）都在跑。

**仍未验证**：你自己点一遍的手感（字体、间距、ECharts 图形的视觉效果）—— 截图只证明「功能通了」。
